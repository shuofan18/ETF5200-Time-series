...
Although the evidence of the unit root hypothesis being rejected by the majority of the series is weakened, after endogenizing the breakpoint selection and correction for small sample bias, it does not necessarily mean that unit root are accepted. Because the low power of the test against trend stationary with broken trend series. 
On the other hand, there are still some series being rejected by @zivot2002, these series passed the toughest test, we can say there are stronger evidence for them not being unit root.
...
pnull <- autoplot(yt_null, xlab="year", ylab="consumer prices")
...
yt_al <- as.ts(yt_al)
pal <- autoplot(yt_al, xlab="year", ylab="consumer prices")
...
As @perron1989 said, "whichever view one adopts cannot be decided by data alone". It is reasonable to consider what happens in the real world and make decisions about how we should use all those relevant information. Perron believes the great crash and oil shock are exogenous to the data generating process, while Zivot thinks differently. Although I think it is more appealing that political or other types of events are related to the economic growth, so it might be inappropriate to view them as exogenous. I do not think Zivot's process is ideal to deal with this endogeneity. The approach they used seems to be purely data manipulation. 
